...
<tr>
    <td colspan="5" class="md:hidden px-2 py-2 border-skin-primary border-b border-t-2">
        <div class="flex justify-between">
            <a href="{{ route('category.form', ['id' => $id]) }}" title="Edit" class="font-bold text-skin-link hover:text-skin-link/90 hover:underline">{!! $name !!}</a>
        </div>
    </td>
</tr>
<tr>
    ...
    <td class="hidden md:table-cell px-2 py-2 border-skin-primary border-b border-r whitespace-nowrap md:whitespace-normal md:min-w-96">
        <a href="{{ route('category.form', ['id' => $id]) }}" title="Edit" class="text-skin-link hover:text-skin-link/90 hover:underline">{!! $name !!}</a>
    </td>
    ...
    <td class="hidden md:table-cell px-2 py-2 border-skin-primary border-b border-r whitespace-nowrap text-center">
        ...
        <a href="{{ route('categories', ['id' => $id]) }}" title="Edit" class="inline-block text-skin-link hover:text-skin-link/90">
            <x-grocery.icon name="view" class="h-6 w-6" />
        </a>


        <a href="{{ route('category.form', ['id' => $id]) }}" title="Edit" class="inline-block text-skin-link hover:text-skin-link/90">
            <x-grocery.icon name="edit" class="h-6 w-6" />
        </a>
        <form action="{{ route('category.delete', $id) }}" method="POST" class="inline-block">
            @csrf
            @method('DELETE')
